...
description: 把书籍知识库（你的书籍库/）的书与核心概念构建成可可视化的「知识图谱 / 知识图谱」。核心数据是一个机器可读的全局关系网络 graph.json，图谱 Demo 直接消费它。触发词：书籍知识图谱、知识图谱、跨书关联、书与书怎么关联、把书做成图谱、图谱、graph.json、relation。同时作为 book-knowledge-card 产出后的强制联动环节——每做完一本书的卡片，必须增量更新 graph.json 并刷新 Demo（不要等所有书做完再补）。
---
...
把 `你的书籍库/` 里的书及其核心概念，构建成可可视化的「知识图谱 / 知识图谱」。核心数据是一个机器可读的全局关系网络 `graph.json`，可视化 Demo（力导向图）直接消费它。
...
- 用户问「书与书怎么关联 / 做成图谱 / 知识图谱 / 跨书关系」→ 直接用本 skill 的 schema 与 Demo。
- 单本书内容拆解（卡片、md）仍归 `book-knowledge-card`；跨书关系层归本 skill。

## 数据架构（真值源）

- `书单数据源.md` —— **唯一真值**：每本书的分类 + 关键词（图谱的输入）。
- `书籍知识图谱模板.html` —— 图谱产物（内嵌数据，双击即开）。
- graph.json 的节点/连线**全部来自 `书单数据源.md`**（每本的分类 + 关键词，共享关键词即自动连边）：改书单 → 跑 `scripts/rebuild_clean.py` 重建，不手改 graph.json。
...
完整节点/连线字段见 [references/schema.md](references/schema.md)。
...
| ② | 每个库只认一个主产物 HTML：**主产物才是准版本**，旧的调试版冻住、禁止反向污染 | v25 |
...
- `references/schema.md`：graph.json 的完整字段规范 + 增量流程细节。
...
- 本地预览：直接双击 `书籍知识图谱.html`（file:// 即可，无需 server）；要改模板时直接在 `书籍知识图谱模板.html` 上改，改完双击验证
- 自检：刷新后点大星能弹出对应知识卡片 = 链路通；不能 = 检查 books/<书名>/<书名>-知识卡片.html 是否存在且文件名含"知识卡片"

## 设计纪律（不照抄古诗图谱）

书籍没有朝代/人名这类硬关系，靠**语义亲疏**聚类：相似→近、互补→中、相悖→远但红线连（对立是刻意设计，不是漏连）。不要照搬「按时间排」的图谱逻辑——书的世界是观点世界，不是编年史。
